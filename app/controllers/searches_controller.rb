...
class SearchesController < ApplicationController
  ...
  def search
    if params[:prodiver].nil?
      flash.now[:notice] = "Please select where you want to search."
      render :search
    else
      if params[:provider] == "vimeo"
        search_term = params[:search]
        vimeo_access_token = ENV["VIMEO_ACCESS_TOKEN"]
        results = HTTParty.get("https://api.vimeo.com/users?page=1&per_page=25&query=#{search_term}&fields=uri,name,bio,pictures",
